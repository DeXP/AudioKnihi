<template>
  <div class="min-h-screen bg-base-100 text-base-content transition-colors">
    <header class="navbar bg-base-200 shadow-sm px-4">
      <div class="navbar-start">
        <router-link to="/" class="btn btn-ghost text-xl no-animation">🎧 АўдыёКнігі</router-link>
      </div>
      <div class="navbar-end gap-2">
        <label class="swap swap-rotate">
          <input type="checkbox" :checked="isDark" @click="toggleTheme" class="theme-controller" />
...
onMounted(() => {
  const saved = localStorage.getItem('audioknihi-theme')
  if (saved) {
...
const applyTheme = () => {
  // ✅ DaisyUI natively uses data-theme, not .dark class
  const theme = isDark.value ? 'dark' : 'light'
  document.documentElement.setAttribute('data-theme', theme)
  localStorage.setItem('audioknihi-theme', theme)
}
